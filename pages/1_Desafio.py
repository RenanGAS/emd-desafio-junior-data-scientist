...
st.markdown(
    """
    Como desafio, propô-se a solução de questões sobre a base de dados do 1746, utilizando SQL, e a criação de visualizações de dados para as perguntas e para 
    análises feitas por conta do candidato.""")

st.write("## Questionário")
st.markdown(
    """
    Para solução das questões foi utilizado o atributo `data_partição` para otimização das consultas. O conjunto se encontra particionado por mês, representando este atributo
    a partição de um chamado. Desta forma, ao definir uma data de partição na cláusula `WHERE`, o espaço de busca de dados diminui consideravelmente.
    
    Acompanhado das respostas está disponível o script SQL feito. Da mesma forma, também se encontra os scripts SQLs para obter os dados exibidos nos gráficos.""")
...
with st.container(border=True):
    ...
    with st.container(border=True):
        st.markdown(
            """
            Destes 73 chamados, 24 são do tipo "Poluição Sonora", 9 são de "Estacionamento Irregular" e 8 de "Iluminação Pública". O restante dos chamados tiveram proporções menores. No gráfico abaixo são exibidos os tipos e a quantidade de chamados de cada um neste dia.""")
    ...
    ans2.markdown(
        """
        Nenhum chamado teve reclamações neste dia. Para verificar esta questão, agrupou-se os chamados por `tipo`, somou-se o campo `reclamacoes` de cada um, fez-se a ordenação decrescente dos tipos por soma de reclamações (do tipo com mais reclamações para o com menos) e selecionou-se o primeiro.""")
    ...
    SELECT tipo AS Tipo, SUM(reclamacoes) AS Numero FROM `datario.administracao_servicos_publicos.chamado_1746` 
        WHERE data_particao = "2023-04-01" AND CAST(data_inicio AS DATE) = "2023-04-01" GROUP BY tipo ORDER BY SUM(reclamacoes) DESC LIMIT 1;
    '''
    ...
    ans3.markdown(
        """
        Os bairros que mais tiveram chamados neste dia foram Engenho de Dentro (8 chamados), Leblon (6 chamados) e Campo Grande (6 chamados). Para obter estes resultados, agrupou-se os chamados por `id_bairro`, ordenou-se por ordem decrescente de quantidade de chamados, e selecionou-se os 3 primeiros. Disto, fez-se o `JOIN` da tabela de bairros com o resultado anterior por equivalência do campo `id_bairro`, encontrando os nomes dos bairros.""")
    ...
    ans4.markdown(
        """
        A subprefeitura com mais chamados abertos neste dia é a Zona Norte com 25 chamados. Utilizou-se o mesmo procedimento da questão anterior, mas selecionando apenas o primeiro resultado e procurando pelo nome da subprefeitura a qual o bairro pertence, ao invés do nome dele, na tabela de bairros.""")
    ...
    with st.container(border=True):
        st.markdown(
            """
            A cidade do Rio de Janeiro possui 8 subprefeituras. Um modo interessante de visualizar a distribuição de chamados por estas regiões é exibindo o mapa do Rio de Janeiro, identificando a localização dos chamados com marcações, e destacando os limites de cada subprefeitura. Com isto em mente, utilizou-se o atributo `geometry` dos chamados para mapeá-los no arquivo `Shapefile` da cidade (tipo especial de arquivo com dados geoespaciais) encontrado no site data.rio (o arquivo continha dados para demarcação das subprefeituras). Abaixo encontra-se o resultado.""")
    ...
    ans5.markdown(
        """
        Sim, existe um chamado sem associação a um bairro. A causa para este chamado em específico (do tipo "Taxi") pode ser pela vulnerabilidade no formulário de cadastro no site 1746.rio, que permite tanto registros sem a informação do bairro, como com valores incorretos. Outro fator pode ser a opção de fazer chamados anônimos, o que impede a associação do bairro em que o usuário mora (obtido no cadastro da conta) ao bairro do chamado. A presença de chamados sem a identificação do bairro pode gerar erros em análises, caso o profissional encarregado não estiver ciente dos diversos fatores que podem afetar a integridade de um conjunto de dados.""")

    exp_sql_ans5 = st.expander("Script SQL")
    sql_ans5 = '''
    SELECT tipo, subtipo, categoria FROM `datario.administracao_servicos_publicos.chamado_1746` 
    ...
    ans6.markdown(
        """
        Dentro deste período foram abertos 42.408 chamados deste subtipo. Para obter este resultado, procurou-se chamados com `data_inicio` dentro deste intervalo de tempo e com o subtipo "Perturbação do sossego".""")
    ...
    with st.container(border=True):
        st.markdown(
            """
            Deste total de 42.408 chamados, xxxx foram feitos em 2022, e xxxx em 2023. Para comparação dos dois anos (2022 e 2023) sobre este aspecto, uma visualização da média diária de chamados deste subtipo por mês, pode ilustrar os períodos no ano de 2023 que tiveram menos chamados em relação a 2022. Com a construção deste gráfico, observou-se que a partir do mês de abril houve uma queda significativa no número de chamados em 2023. Abaixo mostra-se o resultado.""")
    ...
    ans7.markdown(
        """
        Para obter estes chamados, consultou-se as datas destes eventos na tabela de eventos, e procurou-se por chamados deste subtipo que possuiam `data_inicio` dentro dos determinados períodos.""")
    ...
    ans9.markdown(
        """
        O evento que teve a maior média diária de chamados foi o Rock in Rio (2022) com 119 chamados por dia. Em seguida está o Carnaval (de 2023) com 60, e o Reveillon (de 2022) com 45. Para obter este resultado, incrementou-se ao procedimento da questão anterior, o agrupamento dos chamados por `data_inicio` em cada seleção, para soma dos chamados feitos em cada dia, e então feita a operação de média diária a partir destas somas. Uma alternativa mais eficiente seria selecionar todos chamados feitos durante o período de um evento e dividir a quantidade de chamadas pelo número de dias do período, no entanto, a quantidade de dias teria que ser colocada manualmente na consulta ou calculada anteriormente e armazenada numa variável. A primeira solução se torna mais custosa se o agrupamento opera sobre uma quantidade grande de dados, mas evita a contagem de dias numa outra consulta. Para esta questão, ambas soluções consomem 5,05 MB de processamento.""")
    ...
    with st.container(border=True):
        st.markdown(
            """
            Elaborou-se uma comparação da média diária de chamados deste subtipo no Carnaval e Reveillon em 2020, 2022 e 2023. Abaixo mostra-se o resultado. (O ano de 2021 não foi incluído pelo cancelamento do Carnaval)""")
    ...
    ans10.markdown(
        """
        A média diária de chamados deste subtipo entre 01/01/2022 e 31/12/2023 foi de 63. Pode-se afirmar que chegou a este valor por causa do ano de 2022, o primeiro ano pós-pandemia, em que houve a diminuição de restrições quanto a promoção de atividades recreativas de interação social (festas e ocupação de bares e clubes). Observa-se no Gráfico 3. que em 2022 houve uma constância por volta de 80 chamados por dia, com pico de 120 no mês de julho (período de férias escolares). Com isto em mente, pode-se considerar os resultados de 2022 como casos especiais. Nota-se no Gráfico 3. que nos primeiros meses (janeiro, fevereiro, março e abril), a média diária do subtipo é semelhante nos dois anos (2022 e 2023), resultado que pode ser explicado pelo evento de Carnaval.""")
